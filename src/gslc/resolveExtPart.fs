...
let fetchSequence (verbose:bool) (library:Map<string,char array>) (ppp:PPP) (partId:IDAt) =
...
let fetchFullPartSequence (_ (* verbose*):bool) (library:Map<string,char array>) (partId:IDAt) =
// Sequence can come either from the libary or preferably from the hutch directly
    let pid = partId.id
    match legalPartPrefix pid with
...
let getExtPartSlice (verbose:bool) (partId:IDAt) =
    // Start off assuming it's the full DNA slice
    let startSlice =
       {left = {x = 1<OneOffset>; relTo = FIVEPRIME};
        lApprox = false;

...
let expandSimpleMut (asAACheck:bool) (_:GenomeDef) (g:IDAt) (m:Mutation) =
    ...
    | Some(hr) -> // got a hutch rabit
        ...
        | AA ->
            if m.pos <=0 || m.pos > dna.Length/3 then
                failwithf
                    "ERROR: mutation position %d outside range of rabit %s amino acids"
                    m.pos g.id

            let currentCodon = (dna.[(m.pos-1)*3..(m.pos-1)*3+2]).ToCharArray()
            ...
            if not (codon2aa currentCodon = m.f) && asAACheck then
                failwithf
                    ...
                    m.f m.pos m.t g.id (codon2aa currentCodon) (arr2seq currentCodon) m.f
            else
                let id = g.id
                let lhs = m.pos-1
                let rhs = m.pos+1
...
let private classicPromoterNT gene name (f:sgd.Feature) (rg:GenomeDef) (m:Mutation) =
    ...
    let b = (m.pos-1)*1<ZeroOffset> 
    let genomicCoord =
        (if f.fwd then f.l-m.pos else f.r - m.pos (* remember m.pos is negative *) )
      * 1<ZeroOffset>
    ...
    if existing.[0] <> m.f then
        ...
        failwithf
            ...
            name m.pos m.f (existing.[0]) (arr2seq diag)
...
let private classicCodingNT
    ...
    sprintf "%s[1S:%A];/%c/ {#inline };%s[%A:-1E] " g (m.pos-1) m.t g (m.pos+1)
...
let expandAS
    ...
    | AA -> 
        ...
        let b = (m.pos-1) * 3<ZeroOffset> // zero based coordinates, location of mutant codon
        ...
        if not (codon2aa currentCodon = m.f) && asAACheck then
            ...
            failwithf
                "ERROR: for mutation %c%d%c , gene %s has amino acid %c (%s) in that position not %c"
                m.f m.pos m.t g (codon2aa currentCodon) (arr2seq currentCodon) m.f

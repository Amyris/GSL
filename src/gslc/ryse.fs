...
let fetchProxy (proxyBaseURL:string) insertName breed =
        ...
        with _ as ex ->
            failwithf
                "ERROR: from thumper proxy %s\nERROR: Might be rabit id that does not exist.\nERROR: %s\n"
...
let loadRyseLinkers (f:string) =
    if not (File.Exists f) then
        failwithf "ERROR: could not locate reference file '%s'\n" f
...
let loadThumperRef (f:string) =
    if not (File.Exists f) then
        failwithf "ERROR: could not locate thumper reference file%s" f
        ...
        Some(
           {id = int(rabit.Id.[2..]);
            ...
            orient =
                ...
                | "FWD" -> FWD
                | "REV" -> REV
                | _ -> failwithf "ERROR: inconceivable direction %A\n" rabit.Direction;
            breed = rabit.Breed;
        ...
        let (altLinkers1, altLinkers2) =
            ...
            | [| a ; b |] ->
                if invalidLinkerWarn a b then
                    printf
                        "ERROR: linker sets must start with linker 0 and end with linker 9.  %s fails\n"
                        aIn.linkerHint
                    // FIXME: this error condition just prints a message, should it blow up?
                a, b
            | _ ->
                failwithf
                    "ERROR: bad #linkers structure, should be one part with | sep, not %s"
                    (aIn.linkerHint)
...
let mapRyseLinkers
    ...
    let megaMono = aIn.pragmas.ContainsKey("stitch")
    ...
    let rec assign startLinkers (phase:bool) (l:DNASlice list) (linkers: string list) res =
        let prepLinker (n:string) =
            ...
            {id = None;
             ...
             breed = B_LINKER;
             rabitCandidates = Array.empty;}// flag_new_gsl 8/12/15 Added "rabitCandidates"

        let noLinkersLeftMsg =
            sprintf "ERROR: mapRyseLinkers: out of linkers.  Started with %A" startLinkers
        ...
        | [] ->
            if megaMono && linkers.Length > 0 then
                ...
                if phase then // Must be in phase two by the time we get here
                    failwith "ERROR: in mapRyseLinkers:assign, ran out of linkers while still in phase one :(  .  Are you missing a ### marker for your megastitch?"
                ...
                | _ as x ->
                    failwithf
                        "ERROR: mapRyseLinkers: unexpected linker complement  '%s' left at end \nphase=%s (%s)\n"
            ...
            | linkerName::lt ->
                ...
                if hd.sliceType = MARKER then
                    ...
                    if linkersReq > allLinkers2.Length then
                        failwithf
                            "ERROR: mapRyseLinkers - need %d linkers to finish, only %d available %A\n"
                    ...
                    | Some(x) -> //when x.[0] = 'R' || x.[0] = 'r' ->
                        ...
                        let h =
                            if (hutchAncillary.ContainsKey(rabitId)) then hutchAncillary.[rabitId]
                                ...
                                | None ->
                                    failwithf "ERROR:  hutch info missing rabit R%d" rabitId
                                | Some(x) -> x
                        ...
                        let failWithLinkerErrorMsg whichEnd hEnd name =
                            failwithf
                                "ERROR: part R%d expects %s linker (%s) and linker (%s) used instead \nERROR:(%s)"
